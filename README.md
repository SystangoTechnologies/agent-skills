...
```
 ...
 ┌──────┐      ┌───────┐      ┌──────┐      ┌──────┐      ┌───────┐      ┌──────┐
 │Define│ ───▶ │Create │ ───▶ │ Code │ ───▶ │ Unit │ ───▶ │ Code  │ ───▶ │  Go  │
 │ PRD  │      │ Tasks │      │ Impl │      │ Tests│      │Quality│      │ Live │
 └──────┘      └───────┘      └──────┘      └──────┘      └───────┘      └──────┘
...
The installer clones the repo into `~/.cursor/plugins/agent-skills/` and symlinks content into `~/.cursor/{skills,commands,agents}/`. Commands are exposed with the `sys-` prefix (for example `/sys-spec`). Restart Cursor (or start a new agent session) and the skills plus slash commands become available in every project.
